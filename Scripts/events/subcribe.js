...
export default async function ({
  ...
  if (logMessageData.addedParticipants.some((i) => i.userFbId == botID)) {
    ...
    api.changeNickname(
      `[ ${PREFIX} ] ${global.config.NAME || "GBOT - CHIẾN THẦN SÀN WAR 4.0"}`,
      threadID,
      botID
    );
    api.sendMessage(global.config.GBOTWAR_MESSAGE.notiWelecome, threadID);
    /*api.sendMessage(
  ...
  const gifPath = `${global.mainPath}/application/events/subcribeGifs/${threadID}.gif`;
